...
window.game.actions = {
                ...
                var generator   = function(type, bd, text){
                    ...
                    $.each(bd, function(index, id){
                        ...
                        if ( type === 'bye' ){
                            ...
                            id = index;
                            if ( !window.game.items[id]['bye'] ) return;
                        }
    ...
    enemy       : {
        ...
        floatText   : function( type, int, obj, cls ) {
            ...
            var top     = obj.offset().top - this.tap[type].parent().offset().top,
                left    = obj.offset().left - this.tap[type].parent().offset().left,
                text    = $('<span/>',{
                text        : int,
                'class'     : 'floatText' + cls
            }).css({
        ...
        fight       : function(){
            ...
            var game    = window.game,
                ...
                reset   = function(){
                    ...
                    $self.timers.attack = setTimeout(function(){
            ...
            if ( window.game.enemy.hp.real < 1 || window.game.player.hp.real < 1 ) return;
        ...
        defence     : function(){
            ...
            var game    = window.game,
                ...
                reset   = function(){
                    ...
                    $self.timers.defence = setTimeout(function(){
        ...
        isWin       : function(){
            ...
            if ( window.game.enemy.hp.real < 1 ){
                ...
                clearTimeout(this.timers.attack);
                clearTimeout(this.timers.defence);

                var hp      = window.game.enemy.hp.max,
                    gold    = window.game.sys.rand( hp/10, hp),
                    exp     = window.game.sys.rand( hp/10, hp );

                window.game.actions.personage.gold.add(gold);
                window.game.actions.personage.exp.add(exp);

                $('.b-reward__number.gold .b-reward__val').text(gold);
                $('.b-reward__number.exp .b-reward__val').text(exp);
        ...
        getLoot     : function(){
            ...
            var addItem     = function(){
                ...
                G.enemy.reward.push( G.sys.rand( 11, 15) );
            };

            if (G.sys.rand(1, 100) >= 50 ) addItem();
            if (G.sys.rand(1, 100) >= 75 ) addItem();
            if (G.sys.rand(1, 100) >= 95 ) addItem();

            window.game.actions.enemy.renderLoot();
        },


        renderLoot  : function(){

            var $wrap = $('.b-reward__loot');

            $wrap.empty();

            $.each(window.game.enemy.reward, function(index, id){
                ...
                var item    = window.game.items[ id ],
                    ...
                    reward  = $('<img>',{
                        ...
                        window.game.actions.personage.item.add(id);

                        reward.remove();

                        var idx = window.game.enemy.reward.indexOf(id);

                        if (idx != -1) window.game.enemy.reward.splice(idx, 1);
